// src/helpers/pdf-builder.helper.ts
...
export async function buildPDF(
  ...
  // step 1 — compress all images in parallel before PDF starts
  // this is the expensive step — run it all at once
  ...
  // step 2 — set headers before streaming starts
  // we know sizes now because compression is done
  ...
  const doc = new PDFDocument({
    margin: 50,
    autoFirstPage: true,
    bufferPages: false, // stream pages as they're written
    info: {
      Title: 'Market Shop Survey Report',
      Author: 'Noemdek Survey System',
      // no extra metadata — keeps PDF lean
    },
  });

  // pipe directly to response — no buffering
  doc.pipe(res);
  ...
  const stats: [string, any][] = [
    ...
    ['Avg Appliances per Shop', avgAppliances],
    ['Original Image Size', `${Math.round(totalOriginalBytes / 1024)} KB`],
    ['Compressed Image Size', `${Math.round(totalCompressedBytes / 1024)} KB`],
  ];
  ...
  doc
    ...
    .text('Top Energy Source', col.energy, tableTop);
